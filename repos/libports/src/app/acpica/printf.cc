/*
 * \brief  ACPICA printf backend
 *
 * Based on dde_linux printf.cc
 */
...
extern "C" void AcpiOsPrintf (const char *fmt, ...)
{
	va_list va;
	va_start(va, fmt);
	Util::Console::c().vprintf(fmt, va);
	va_end(va);
}

extern "C" void AcpiOsVprintf (const char *fmt, va_list va)
{
	Util::Console::c().vprintf(fmt, va);
}
