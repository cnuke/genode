...
class Gpu::Call
{
	private:
		...
		Gpu_connection _open_gpu()
		{
			Gpu_connection con { .fd = ::open("/dev/gpu", 0), .id = 0 };
			if (con.fd < 0) {
		...
		Gpu_connection _con { _open_gpu() };

		Gpu::Connection &_gpu_session { *vfs_gpu_connection(_con.id) };
		Gpu::Info_lima const &_gpu_info {
			*_gpu_session.attached_info<Gpu::Info_lima>() };

		Genode::Io_signal_handler<Call> _completion_handler {
			_env.ep(), *this, &Call::_handle_completion };
		...
		void _wait_for_mapping(uint32_t handle, unsigned op)
		{
			...
			do {
				if (_gpu_session.set_tiling(id, op))
					break;

				char buf;
				::read(_con.fd, &buf, sizeof(buf));
			} while (true);
		}
