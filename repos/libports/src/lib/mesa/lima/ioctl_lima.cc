...
class Gpu::Call
{
	private:
		...
		using Gpu_context_id    = Genode::Id_space<Gpu_context>;
		...
		void _wait_for_mapping(uint32_t handle, unsigned op)
		{
			...
			do {
				...
				char buf;
				::read(_gpu_session.fd, &buf, sizeof(buf));
			} while (true);
		}
			...
			_gpu_context_space.apply<Gpu_context>(ctx_id, [&] (Gpu_context &gc) {
				...
				try {
					...
					Genode::uint64_t const pending_exec_buffer =
						gpu.exec_buffer(_exec_buffer->id(), EXEC_BUFFER_SIZE).value;
		...
		int _drm_lima_ctx_free(drm_lima_ctx_free &arg)
		{
			...
			_gpu_context_space.apply_any<Gpu_context>(free_ctx);
		...
		{
			try {
				Gpu::Syncobj_id const id = _gpu.create_syncobj();
				if (!id.valid) {
		...
		{
			_gpu.destroy_syncobj(Gpu::Syncobj_id { .value = arg.handle,
			                                                .valid = true });
			return 0;
		}
	...
	public:
		...
		~Call()
		{
			_gpu_context_space.apply_any<Gpu_context>([&] (Gpu_context &ctx) {
				Genode::destroy(_heap, &ctx);
			});
		}
