...
struct Vfs::Resolvconf_file_system : Single_file_system
{
	...
	struct Resolvconf_vfs_handle : Single_vfs_handle
	{
		...
		Read_result read(Byte_range_ptr const &dst, size_t &out_count) override
		{
			...
			try {
				/**
				 * Since vfs_lxip pseudo files announce a size of 4096 bytes
				 * we set the limit to the same amount to prevent
				 * Truncation_during_read exceptions due to the failed size
				 * check.
				 */
	...
	Stat_result stat(char const *path, Stat &out) override
	{
		...
		static size_t const nameserver_string_len = strlen(_nameserver_string);
		out.size += nameserver_string_len;
