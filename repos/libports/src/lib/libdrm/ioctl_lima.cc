...
static constexpr bool verbose_ioctl = true;
	...
	private:
		...
		struct Gpu_context
		{
			private:
				...
				template <typename FN>
				void _try_apply(Genode::Id_space<Buffer>::Id id, FN const &fn)
				{
					try {
				 ...
				 * command buffer.
				 *
				 * Use constructible as we need to destruct the exec buffer
				 * before we close the Gpu session.
				 */
				Constructible<Gpu::Vram> _exec_buffer { };
			...
			public:
				...
				:
					...
					_va_alloc { va_alloc }
				{
					_exec_buffer.construct(_gpu, _exec_buffer_size,
				...
				void free_buffer(Genode::Id_space<Buffer>::Id id)
				{
				...
				bool buffer_space_contains(Genode::Id_space<Buffer>::Id id)
				{
					bool found = false;
		...
		struct Syncobj
		{
			struct Invalid_gpu_context : Genode::Exception { };
		...
		Gpu_context _main_ctx { _heap, _gpu_context_space, _va_alloc };
		...
		int _wait_for_syncobj(int fd)
		{
			...
			try {
				 ...
				 * submission.
				 */
				Genode::warning("ignore invalid GPU context for fd: ", fd);
				return -1;
			}
			...
			_syncobj_space.apply<Syncobj>(syncobj_id, [&] (Syncobj &sync_obj) {
				...
				_gpu_context_space.apply<Gpu_context>(ctx_id, [&] (Gpu_context &gc) {
					...
					Lima::for_each_submit_bo(arg, [&] (drm_lima_gem_submit_bo const &bo) {
						Genode::Id_space<Buffer>::Id const id = { .value = bo.handle };
					...
					gc.access_exec_buffer(_env, [&] (char *ptr, size_t size) {
						...
						if (payload_size > size) {
							Genode::error("exec buffer for context ", ctx_id.value,
		...
		int _drm_gem_close(drm_gem_close const &gem_close)
		{
			auto free_buffer = [&] (Gpu_context &ctx) {
				Genode::Id_space<Buffer>::Id const id { .value = gem_close.handle };
				ctx.free_buffer(id);
			};
