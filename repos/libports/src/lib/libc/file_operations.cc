...
extern "C" ssize_t aio_return(struct aiocb *iocb)
{
	...
	int error = EINVAL;
	ssize_t result = 0;
	fd->apply_lio(iocb, [&] (File_descriptor::Lio_slot &slot) {
		using LSS = File_descriptor::Lio_slot::State;
		...
		case LSS::COMPLETE:
			...
			result = slot.result;
			break;
		case LSS::FREE:        error = EINVAL; break;
		case LSS::IN_PROGRESS: error = EINVAL; break;
		case LSS::PENDING:     error = EINVAL; break;
		}
...
extern "C" int aio_error(const struct aiocb *iocb)
{
	...
	fd->apply_lio(iocb, [&] (File_descriptor::Lio_slot &slot) {
		...
		switch (slot.state) {
		case LSS::COMPLETE:    error = slot.error;  break;
		case LSS::FREE:        error = EINVAL;      break;
		case LSS::IN_PROGRESS: error = EINPROGRESS; break;
		case LSS::PENDING:     error = EINPROGRESS; break;
		}
	});
                           ...
                           const struct timespec *timeout)
{
	...
	/* XXX change once_okay hack (better result in wait_aio */
	bool once_okay = false;
	int result = -1;
	int libc_fd = -1;
