...
Rpc_entrypoint::_alloc_rpc_cap(Platform &platform, Native_capability ep, addr_t entry)
{
	if (!_native_pd_cap.valid())
		_native_pd_cap = platform.pd.native_pd();
...
{
	return platform.pd.free_rpc_cap(cap);
}
