...
struct Nova_vcpu : Rpc_client<Vm_session::Native_vcpu>, Noncopyable
{
	private:
		...
		inline void _read_nova_state(Nova::Utcb &, uint8_t const &);
		...
		void _handle_exit(Nova::Utcb &, uint8_t const &);
...
void Nova_vcpu::_read_nova_state(Nova::Utcb &utcb, uint8_t const &fpu_at_exit)
{
 ...
 */
void Nova_vcpu::_handle_exit(Nova::Utcb &utcb, uint8_t const &fpu)
{
...
void Nova_vcpu::with_state(Call_with_state &cw)
{
	...
	if (remote) {
		...
		uint8_t fpu_at_exit[512] __attribute__((aligned(0x10)));
		...
		if (utcb.mtd & Nova::Mtd::FPU)
			asm volatile ("fxsave %0" : "=m" (*fpu_at_exit) :: "memory");
		_read_nova_state(utcb, *fpu_at_exit);
	}
...
void Nova_vcpu::_exit_entry(addr_t badge)
{
	...
	try {
		...
		{
			vcpu._handle_exit(utcb, *_fpu_at_exit);
