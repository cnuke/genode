                                ...
                                public Genode::Registry<Gpu::Session_component>::Element
{
	private:
		...
		static int _convert_ma(Gpu::Mapping_attributes attrs)
		{
			if (attrs.readable)
				return 1;
			else
		...
		Gpu::Sequence_number _pending_seqno { };
		...
		static void _drm_worker_run(void *p)
		{
			...
			while (true) {
				...
				auto dispatch_pending = [&] (Gpu::Request r) {
					...
					{
						uint32_t const handle = registry.lookup_handle(r.operation.id);
						int const attrs = _convert_ma(r.operation.mapping_attrs);

						int const err =
							lx_drm_ioctl_etnaviv_cpu_prep(args.drm_session, handle, attrs);
						if (err) {
							break;
						}
	...
	public:
		                                 ...
		                                 Genode::size_t size) override
		{
			...
			if (_completed_request.success) {
				seqno = Gpu::Sequence_number {
					.value = _completed_request.operation.seqno.value };
			} else {
		...
		bool complete(Gpu::Sequence_number seqno)
		{
			Gpu::Request r = Gpu::Request::initialize(Gpu::Operation::Type::WAIT);
			r.operation.seqno = Gpu::Seqno { .value = _pending_seqno.value };
