...
static struct Handle_id_array const *_global_ids;
...
static void lx_drm_gem_submit_in(struct drm_etnaviv_gem_submit *submit)
{
	...
	for (i = 0; i < submit->nr_bos; i++) {
		...
		struct drm_etnaviv_gem_submit_bo *bo =
			&((struct drm_etnaviv_gem_submit_bo*)submit->bos)[i];

		for (j = 0; j < _global_ids->count; j++) {
			if (bo->handle == (_global_ids->items[j].id & 0xffffffffu)) {
				bo->handle = _global_ids->items[j].handle;
			}
		}
	}
}
...
int lx_drm_ioctl_etnaviv_gem_submit(void *session, unsigned long arg,
                                    unsigned int *fence,
                                    struct Handle_id_array const *ids)
{
	int err;
