...
class Lx_kit::Irq : public Lx::Irq
{
	private:
		...
		class Handler : public Lx_kit::List<Handler>::Element
		{
			private:
				...
				handler_t  _handler;   /* Linux handler */
				handler_t  _thread_fn; /* Linux thread function */

			public:

				Handler(void *dev, unsigned int irq, handler_t handler,
				        handler_t thread_fn)
				...
				bool handle()
				{
					return genode_emul_interrupt_handler(_dev, _irq, _handler, _thread_fn);
				}
					// if (!_handler) {
	...
	public:
		...
		class Context : public Lx_kit::List<Context>::Element
		{
			private:
				...
				Genode::Io_signal_handler<Context> _dispatcher;
	...
	public:
		...
		void request_irq(Genode::Irq_session_capability cap,
		                 ...
		                 handler_t                  handler,
		                 void                         * dev_id,
		                 handler_t                  thread_fn = 0) override
		{
