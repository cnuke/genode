...
struct Vfs::Oss_file_system::Audio
{
	...
	private:
		...
		struct Output
		{
			...
			Output(Genode::Env &env) : _env { env } { }
			...
			unsigned samples_per_channel() const { return _samples; }
		};
		...
		void _try_schedule_and_enqueue_once(Stereo_output &output)
		{
			if (!output.wait_for_timer())
		...
		struct Input
		{
			...
			Input(Genode::Env &env) : _env { env } { }

			void update_parameters(unsigned            const duration_us,
			                       Record::Num_samples const num_samples)
			{
				_timer_duration_us = duration_us;
			...
			void timer_start() {
				_timer.start(_timer_duration_us); }
	...
	public:
		...
		void update_input_duration(unsigned const bytes)
		{
			_with_input([&] (Stereo_input &input) {
				_with_duration(bytes, [&] (unsigned const duration, unsigned const samples) {
					input.update_parameters(duration, Record::Num_samples { samples });
				});
			});
		}
		...
		void update_output_duration(unsigned const bytes)
		{
			_with_stereo_output([&] (Stereo_output &output) {
				_with_duration(bytes, [&] (unsigned const duration, unsigned const samples) {
					_stereo_output_underrun_limit = 1'000'000u / duration;
					output.update_parameters(Play::Duration { duration }, samples);
				});
			});
		}
		...
		bool handle_play_timer()
		{
			...
			_with_stereo_output([&] (Stereo_output &output) {
				...
				output.wait_for_timer(false);

				error("play_timer: avail: ", output._session_buffer[0].used());
		...
		void enable_output(bool enable)
		{
			_with_stereo_output([&] (Stereo_output &output) {
				...
				else                 _try_schedule_and_enqueue_once(output);
			});
		}
		...
		bool write_ready() const
		{
			...
			_with_stereo_output([&] (Stereo_output const &output) {
				...
				result = output.space_avail  (  samples_per_channel)
				     && !output.samples_avail(1*samples_per_channel);
			});
			return result;
		}
		...
		Write_result write(Const_byte_range_ptr const &src, size_t &out_size)
		{
			...
			auto sample_count = [&] (Const_byte_range_ptr const &range) {
				return (unsigned)range.num_bytes
				     / (_format_size(_info.format) * _info.channels);
			};
			...
			_with_stereo_output([&] (Stereo_output &output) {
				...
				error("write: avail: ", output._session_buffer[0].used());
				...
				if (output.samples_avail(1 * output.samples_per_channel())) {
					result = Write_result::WRITE_ERR_WOULD_BLOCK;
					return;
				}

				error("write: avail: ", output._session_buffer[0].used(), " samples: ", samples);
				for (unsigned i = 0; i < _info.channels; i++)
					output.consume(Stereo_output::Channel { i }, src, samples);
				 ...
				 * scheduling.
				 */
				_try_schedule_and_enqueue_once(output);
