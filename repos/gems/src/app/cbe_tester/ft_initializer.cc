...

{
	...
	case CS::INIT_NODE:
		...
		case Channel::BLOCK_ALLOC_COMPLETE:
			...
			Ft_initializer_channel::reset_node(child);
			child.pba = channel._blk_nr;
			sha256_4k_hash(&child_level.children, child.hash);
                               ...
                               bool    &progress)
{
	...
	for (Genode::uint64_t child_idx = 0; child_idx <= req._max_child_idx; child_idx++) {
		...
		if (state != Ft_initializer_channel::Child_state::DONE) {
			...
			Cbe::Type_2_node &child =
				channel._t2_level.children.value[child_idx];
	...
	for (Genode::uint64_t level_idx = 1; level_idx <= req._max_level_idx; level_idx++) {
		...
		for (Genode::uint64_t child_idx = 0; child_idx <= req._max_child_idx; child_idx++) {
			...
			if (state != Ft_initializer_channel::Child_state::DONE) {
				...
				Cbe::Type_1_node &child =
					channel._t1_levels[level_idx].children.value[child_idx];
                                    ...
                                    bool    &progress)
{
	...
	case Channel::SUBMITTED:
		...
		for (unsigned int i = 0; i < TREE_LEVEL_MAX; i++) {
