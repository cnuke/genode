...
void Sculpt::gen_wifi_drv_start_content(Xml_generator &xml)
{
	...
	xml.node("route", [&] () {
		...
		using Wifi_drv_string = String<32>;
		...
		gen_parent_rom_route(xml, "wifi.lib.so",
			Wifi_drv_string { Sculpt::Feature::WIFI_DRV_PREFIX, "_wifi.lib.so" });
		gen_parent_rom_route(xml, "wifi_firmware.tar",
			Wifi_drv_string { Sculpt::Feature::WIFI_DRV_PREFIX, "_wifi_firmware.tar" });
