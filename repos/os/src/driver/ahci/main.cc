...
struct Ahci::Block_session_component : Rpc_object<Block::Session>
{
	...
	static Block::Session::Info sanatize_info(Block::Session::Info const &info,
	...
	:
		...
		_request_stream(_env.rm(), _dma_cap, _env.ep(), sigh,
		                sanatize_info(_port.info(), block_range)),
...
struct Ahci::Port_dispatcher
{
	...
	Session_capability new_session(Block::Range const &block_range,
	                               size_t              tx_buf_size)
	{
	                            ...
	                            Affinity const &) override
	{
		...
		Block::Range const block_range {
			...
			.num_blocks = Arg_string::find_arg(args.string(), "num_blocks")
			                                   .ulonglong_value(0),
			.writeable  = port.info().writeable && writeable_arg
		};

		try {
			return port_dispatcher[port.index]->new_session(block_range, tx_buf_size);
