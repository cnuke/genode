...
struct Config_helper
{
	...
	void apply_quirks()
	{
		...
		enum {
			ETHERNET_CLASS_CODE      = 0x20000,
			OTHER_NETWORK_CLASS_CODE = 0x28000,
			USB3_CLASS_CODE          = 0xc0330,
		};

		if (   (_cfg.class_code & 0xffff00u) == ETHERNET_CLASS_CODE
		    || (_cfg.class_code & 0xffff00u) == OTHER_NETWORK_CLASS_CODE
		    || (_cfg.class_code & 0xffffffu) == USB3_CLASS_CODE) {
