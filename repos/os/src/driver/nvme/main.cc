                         ...
                         Platform::Device::Irq
{
	 ...
	 * Delete I/O queue
	 */
	void delete_io(uint16_t cid, uint16_t sid)
	{
		_delete_io_sq(sid);
		_delete_io_cq(cid);
	}
	 ...
	 */
	bool active_io(uint16_t qid)
	{
		/* SQ implies working CQ */
		return _sq[qid].constructed();
	}
	 ...
	 */
	Byte_range_ptr io_command(uint16_t qid, uint16_t cid)
	{
		Nvme::Sq &sq = *_sq[qid];
	 ...
	 */
	bool io_queue_full(uint16_t qid) const
	{
		Nvme::Sq const &sq = *_sq[qid];
		Nvme::Cq const &cq = *_cq[qid];
	...
	void handle_io_completion(uint16_t qid, FUNC const &func)
	{
		if (!_cq[qid].constructed())
			return;
                                       ...
                                       Block::Request_stream
{
	...
	:
		Request_stream { env.rm(), io_queue.dma_cap(),
		                 env.ep(), sigh, info },
		_env         { env },
		_io_queue    { io_queue },
		_info        { info },
...
class Nvme::Driver : Genode::Noncopyable
{
	...
	public:
		...
		void reinit(Nvme::Controller &ctrlr)
		{
			...
			for (uint16_t qid = 1; qid < NUM_QUEUES; qid++) {
		...
		using Io_queue_create_result = Attempt<Io_queue,
		                                       Io_queue_creation_error>;

		Io_queue &create_io_queue(size_t tx_buf_size)
		{
			Io_queue::Id const new_id { .value = uint16_t(_io_queue_map.alloc() + 1) };
			bool success = false;
			try {
				with_nvme([&](auto &ctrlr) {
					ctrlr.setup_io(new_id.value, new_id.value);
					success = true;
				}, [&]() {
					error("unexepected NVME controller state - create_io_queue");
				});
			} catch (...) { _io_queue_map.free(new_id.value - 1); }

			if (!success)
				throw Io_queue_creation_error();

			try {
				return *new (_sliced_heap) Io_queue(new_id, _platform,
					                             tx_buf_size);
			} catch (...) { }
...
struct Nvme::Main : Rpc_object<Typed_root<Block::Session>>
{
	                            ...
	                            Affinity const &) override
	{
		...
		try {
			_block_session.construct(_env, _driver.create_io_queue(tx_buf_size),
			                         _request_handler,
			                         _driver.info(),
			                         block_range);
		} catch (Driver::Io_queue_creation_error) { }
