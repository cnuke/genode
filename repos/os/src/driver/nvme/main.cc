...
struct Nvme::Io_queue : Noncopyable
{
	...
	using Command_id = Bitmap<Nvme::MAX_IO_ENTRIES>;
	...
	Io_queue(Io_queue_space       &space,
	         ...
	         size_t                tx_buf_size)
	:
		_elem            { *this, space, Io_queue_space::Id { .value = id } },
                                       ...
                                       Block::Request_stream
{
	...
	Block_session_component(Session_space              &space,
	                        ...
	                        uint16_t                    session_id)
	:
		Request_stream { env.rm(), queue_dma_cap, env.ep(), sigh, info },
		_env         { env },
	...
	{
		/* adjust info to constraint range */
		uint64_t const num_blocks =
			_block_range.num_blocks ? _block_range.num_blocks
			                        : ~0ull;
		_info.block_count = min(_info.block_count, num_blocks);
		_info.writeable   = _block_range.writeable;
	...
	public:
		             ...
		             Nvme::Controller &ctrlr)
		{
			...
			if (len > Nvme::MPS && !need_list) {
				...
				addr_t pa = io_queue.prp_dma_addr() + offset;
				addr_t va = io_queue.prp_addr() + offset;
		...
		Response submit(Io_queue &io_queue, Block::Request const &request,
		...
		Bitmap<MAX_IO_QUEUES> _io_queue_map   { };
		...
		Io_queue_create_result create_io_queue(size_t tx_buf_size)
		{
			bool success = false;
			uint16_t const new_id = uint16_t(_io_queue_map.alloc() + 1);
			try {
				with_nvme([&](auto &ctrlr) {

					ctrlr.setup_io(Io_queue_space::Id { .value = new_id },
					               Io_queue_space::Id { .value = new_id });
					success = true;
					new (_sliced_heap) Io_queue(_io_queue_space, new_id,
					                            _platform, tx_buf_size);
				}, [&]() {
					error("unexepected NVME controller state - create_io_queue");
				});
			} catch (...) { _io_queue_map.free(new_id - 1); }

			if (success)
				return Io_queue_space::Id { .value = new_id };

			return Io_queue_creation_error();
		}

		void free_io_queue(Io_queue_space::Id id)
		{
			with_nvme([&](auto &ctrlr) {
...
struct Nvme::Main : Rpc_object<Typed_root<Block::Session>>
{
	...
	Bitmap<MAX_IO_QUEUES> _session_map { };
			...
			_driver.with_io_queue(block_session.queue_id(), [&] (Io_queue &io_queue) {
				...
				for (;;) {
					...
					block_session.with_requests([&] (Block::Request request) {
						...
						Response response = _driver.submit(io_queue, request);
					...
					/* process I/O */
					progress |= _driver.commit_pending_submits(io_queue);
	                            ...
	                            Affinity const &) override
	{
		...
		_driver.create_io_queue(tx_buf_size).with_result(
			[&] (Io_queue_space::Id queue_id) {
