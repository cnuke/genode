                                       ...
                                       Bdf bdf, unsigned bar, uint64_t addr, uint64_t size)
{
	...
	auto const subsystem_device_id = cfg0.read<C0::Subsystem_device>();
	...
	if  (subsystem_vendor_id == 0x1e26 /* Fujitsu */
	 || (subsystem_vendor_id == 0xf111 && subsystem_device_id == 0x0002) /* Framework 12th Gen */) {
	...
	if (subsystem_vendor_id == 0xf111 && subsystem_device_id == 0x0009 /* Framework 14th Gen */) {
		if (bdf == Bdf { 0, 0x15, 0 } && bar == 0) base_address = 0x60192f9000;
		if (bdf == Bdf { 0, 0x15, 1 } && bar == 0) base_address = 0x60192fa000;
	}

	if (subsystem_vendor_id == 0x17aa && device_id == 0x51e8 /* Lenovo X1 Nano Gen 2 */)
		if (bdf == Bdf { 0, 0x15, 0 } && bar == 0) base_address = 0x4107000000;
...
void Main::parse_acpi_device_info(Node const &node, Generator &g)
{
	...
	g.node("device", [&]
	{
		/* also known as INTC105E for Arrow Lake */
