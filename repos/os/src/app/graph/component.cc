...
class Graph
{
	private:
		...
		Genode::uint16_t              _column_offset { 0 };
		...
		bool                          _sliding { false };
		bool                          _verbose { false };
		Genode::uint64_t              _freq_khz { 2000000 };
...
*/
		...
		Gui::Point _apply_data_point(unsigned const value, unsigned element);
		...
		Genode::uint64_t _time(unsigned const pos) const {
...
Gui::Point Graph::_apply_data_point(unsigned const value, unsigned element)
{
...
void Graph::_handle_input()
{
	...
	{
		Genode::uint64_t const ms = _time(column) / _freq_khz;
		Genode::uint64_t const s = ms / 1000;
		Genode::uint64_t const m = s / 60;
