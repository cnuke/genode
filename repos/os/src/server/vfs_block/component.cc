...
class Vfs_block::File
{
	private:
		...
		Block::block_number_t _allowed_block_count { 0 };
	...
	public:
			...
			_view        { view }
		{
			...
			uint64_t file_block_count = stat.size / info.block_size;

			uint64_t const num_blocks =
				view.num_blocks.value ? view.num_blocks.value : ~0ull;
			_allowed_block_count = min(file_block_count, num_blocks);

			_block_info = Block::Session::Info {
				.block_size  = info.block_size,
				.block_count = _allowed_block_count,
		...
		bool valid(Block::Request const &request)
		{
			...
			bool const valid_range = op.count
			                      && (op.block_number + op.count)
			                      <= _allowed_block_count;
			switch (op.type) {
			case Type::WRITE:
