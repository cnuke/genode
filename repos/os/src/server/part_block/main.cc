...
class Block::Main : public Sync_read::Handler
{
	private:
		...
		struct Partition_number
		{
			...
			static Partition_number from_xml(Xml_node        const &node,
		...
		void _handle_session_request(Xml_node const &request);
		                                ...
		                                Partition_number    const  partition)
		{
			...
			if (tx_buf_size > ram_quota.value) {
				error("insufficient 'ram_quota', got ", ram_quota, ", need ",
				     tx_buf_size);
				return Parent::INSUFFICIENT_RAM_QUOTA;
			}
			...
			if (Session::CAP_QUOTA > cap_quota.value) {
				error("insufficient 'cap_quota', got ", cap_quota, ", need ",
				     tx_buf_size);
				return Parent::INSUFFICIENT_CAP_QUOTA;
			}
			...
			Block::Range const range {
				.offset     = size_t(_partition_table.partition_lba(partition.value)),
				.num_blocks = size_t(_partition_table.partition_sectors(partition.value)),
		                                  ...
		                                  auto     const &close_fn)
		{
			...
			if (request.has_type("create") && request.has_sub_node("args")) {

				Session_state::Args const &args =
					request.sub_node("args").decoded_content<Session_state::Args>();

				Affinity const &affinity = Affinity::from_xml(request);
...
void Block::Main::_handle_session_request(Xml_node const &request)
{
	...
	auto create_fn = [&] (Parent::Server::Id  const  server_id,
		...
		auto match_fn = [&] (Xml_node const &policy) {
			...
			if (!partition.valid() || _sessions[partition.value].constructed()) {
				...
				_env.parent().session_response(server_id, Parent::SERVICE_DENIED);
				return;
			}
		...
		with_matching_policy(label, _config.xml(), match_fn, no_match_fn);
	};
	...
	auto upgrade_fn = [&] (Parent::Server::Id const server_id,
		...
		auto upgrade_session_fn = [&] (Forwarded_session &session) {
			...
			_env.parent().session_response(server_id, Parent::SESSION_OK);
		};
	...
	auto close_fn = [&] (Parent::Server::Id const server_id) {
		...
		auto close_session_fn = [&] (Forwarded_session &session) {
			...
			_env.parent().session_response(server_id, Parent::SESSION_CLOSED);
		};
