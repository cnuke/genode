...
struct Vbox::Pci : Platform::Connection,
                   Util::Dma_allocator
{
	struct Missing_controller : Genode::Exception { };

	enum {
		CLASS_GENERIC_SYSTEM_PERIPHERAL = 0x080000u,
		CLASS_MASK                      = 0xffff00u,
		SUBCLASS_SYSTEM_PERIPHERAL      = 0x000800u,
		VBOX_DEVICE        = CLASS_GENERIC_SYSTEM_PERIPHERAL | SUBCLASS_SYSTEM_PERIPHERAL,
		VBOX_BASE_ID       = 0,
	};
	...
	Pci(Genode::Env &env) : Platform::Connection(env)
	{
		...
		_res = _device->resource(VBOX_BASE_ID);
		...
		Genode::log("VBox PCIe device found (",
