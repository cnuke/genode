                                       ...
                                       Block::Request_stream
{
	...
	Block::Geometry _geom;

	Block_session_component(Env &env, Dataspace_capability ds,
	                        Signal_context_capability sigh,
	                        Block::Geometry geom)
	:
		Request_stream(env.rm(), ds, env.ep(), sigh, geom.block_size), _env(env),
		_geom(geom)
	{
		_env.ep().manage(*this);
	}
	...
	void info(Block::sector_t *count, size_t *size, Operations *ops) override
	{
		*count = _geom.block_count;
		*size  = _geom.block_size;
		*ops   = Operations();
...
class Nvme::Driver : Genode::Noncopyable
{
	...
	private:
		...
		struct Io_buffer_mapper
		{
			...
			bool _bitmap_find_free(size_t bits) const
			{
				try {
					_bitmap.find_free(bits);
					return true;
				}
				catch (...) { return false; }
			}

			bool acceptable(size_t size) const
			{
				size_t const bits = size / MPS;
				return !!_buffers.available() && _bitmap_find_free(bits);
			}
		...
		Block::Request_stream::Payload _payload { };
	...
	public:
		...
		: _env(env), _alloc(alloc)
		{
			...
			_geom.block_count = nsinfo.count;
			_geom.block_size  = nsinfo.size;
		...
		void payload(Block::Request_stream::Payload const &payload)
		{
			_payload = payload;
		}
		...
		Block::Geometry geometry() const { return _geom; }
		...
		{
			if (request.count * _geom.block_size > Nvme::MAX_IO_LEN) {
				error("request too large (max:", (size_t)Nvme::MAX_IO_LEN, " bytes)");
				return false;
			}

			if (_nvme_ctrlr->io_queue_full(Nvme::IO_NSID)) {
				error("no free I/O command entries left");
				return false;
			}

			size_t const len       = request.count * _geom.block_size;
			size_t const mps       = _nvme_ctrlr->mps();
			bool   const need_list = len > 2 * mps;
			if (!_io_mapper->acceptable(len + (mps * need_list))) {
				error("I/O mapper cannot map: ", len, " + ", mps * need_list);
				return false;
			}

			Genode::size_t  const count   = request.count;
			Block::sector_t const lba     = request.block_number;
			Block::sector_t const lba_end = lba + count - 1;

			// XXX check only in case of R/W?
			auto overlap_check = [&] (Request const &req) {
				Block::sector_t const start = req.block_request.block_number;
				Block::sector_t const end   = start + req.block_request.count - 1;
			...
			if (_requests.for_each(overlap_check)) { return false; }
		...
		void _submit(Block::Request const request)
		{
			...
			bool const write = request.operation == Block::Request::Operation::WRITE;

			if (write) {
				_payload.with_content(request, [&] (Genode::addr_t addr, Genode::size_t size) {
					char const *buffer = reinterpret_cast<char const*>(addr);
					Genode::memcpy((void*)iob->va, buffer, size);
				});
			}
		...
		void _get_completed_request(Block::Request &out, uint32_t &out_id)
		{
			...
			_nvme_ctrlr->handle_io_completions(Nvme::IO_NSID, [&] (Nvme::Cqe const &b) {
				...
				Request *r = _requests.lookup([&] (Request &r) { return r.id == id; });
				if (!r) {
				...
				out = r->block_request;
				out.success = Nvme::Cqe::succeeded(b) ? Block::Request::Success::TRUE
				                                      : Block::Request::Success::FALSE;

				bool const read = out.operation == Block::Request::Operation::READ;
				if (out.success == Block::Request::Success::TRUE && read) {
					_payload.with_content(out, [&] (Genode::addr_t addr, Genode::size_t size) {
						char *buffer = reinterpret_cast<char *>(addr);
						// XXX size < out.size?
						Genode::memcpy(buffer, (void*)r->iob->va, size);
					});
				}
			}, true);
		}
...
struct Nvme::Main : Rpc_object<Typed_root<Block::Session>>
{
		...
		for (;;) {
			...
			block_session.with_requests([&] (Block::Request request) {
				...
				if (!_driver->acceptable(request)) {
					Genode::error(__func__, ":", __LINE__, "           CANNOT ACCEPT JOB");
			...
			/* process I/O */
			bool const executed = _driver->execute();
			progress |= executed;
	                            ...
	                            Affinity const &) override
	{
		...
		_block_session.construct(_env, _block_ds->cap(), _request_handler,
		                         _driver->geometry());

		_driver->payload(_block_session->payload());
