...
{ \
	...
	SIMD:: FN(dst.pixels, dst.w/16, src.pixels, W, H); \
	Slow:: FN(ref.pixels, ref.w/16, src.pixels, W, H); \
	log(#FN, ":\n", dst); \
	if (dst != ref) { \
		error("", #FN, " failed"); \
		throw 1; \
...
{ \
	Image<DST_W, DST_H> dst { }, ref { }; \
	SIMD:: FN(dst.pixels, dst.w/16, src.pixels, src.w/16, W, H); \
	Slow:: FN(ref.pixels, ref.w/16, src.pixels, src.w/16, W, H); \
	log(#FN, ":\n", dst); \
	if (dst != ref) { \
		error("", #FN, " failed"); \
		throw 1; \
...
static void test_simd_b2f()
{
	...
	TEST_LANDSCAPE ( SIMD, B2f       ::r0, 48, 32, 1, 2 );
	TEST_LANDSCAPE ( SIMD, B2f_flip  ::r0, 48, 32, 1, 2 );
	TEST_PORTRAIT  ( SIMD, B2f      ::r90, 32, 48, 2, 1 );
	TEST_PORTRAIT  ( SIMD, B2f_flip ::r90, 32, 48, 2, 1 );
	TEST_LANDSCAPE ( SIMD, B2f     ::r180, 48, 32, 1, 2 );
	TEST_LANDSCAPE ( SIMD, B2f_flip::r180, 48, 32, 1, 2 );
	TEST_PORTRAIT  ( SIMD, B2f     ::r270, 32, 48, 2, 1 );
	TEST_PORTRAIT  ( SIMD, B2f_flip::r270, 32, 48, 2, 1 );
}
...
static void test_b2f_dispatch()
{
	...
	log("offset calculation of destination window");
	{
		...
		test(texture_landscape, rect, Rotate::R0,   Flip { },
		     expected(y*640 + x, 40, src_landscape_ptr, 40, 4, 3));
		test(texture_landscape, rect, Rotate::R0,   Flip { true },
		     expected(y*640 + 640 - w - x, 40, src_landscape_ptr, 40, 4, 3));
		test(texture_portrait, rect, Rotate::R90,  Flip { },
		     expected(x*640 + 640 - h - y, 40, src_portrait_ptr, 30, 4, 3));
		test(texture_portrait, rect, Rotate::R90,  Flip { true },
		     expected(x*640 + y, 40, src_portrait_ptr, 30, 4, 3));
		test(texture_landscape, rect, Rotate::R180, Flip { },
		     expected((480 - y - h)*640 + 640 - x - w, 40, src_landscape_ptr, 40, 4, 3));
		test(texture_landscape, rect, Rotate::R180, Flip { true },
		     expected((480 - y - h)*640 + x, 40, src_landscape_ptr, 40, 4, 3));
		test(texture_portrait, rect, Rotate::R270, Flip { },
		     expected((480 - x - w)*640 + y, 40, src_portrait_ptr, 30, 4, 3));
		test(texture_portrait, rect, Rotate::R270, Flip { true },
		     expected((480 - x - w)*640 + 640 - y - h, 40, src_portrait_ptr, 30, 4, 3));
	}
	...
	/* snap to grid */
	log("snap rect argument to 16x16 grid");
	test(texture_landscape, { { 31, 63 }, { 2, 2 } }, Rotate::R0, Flip { },
	     expected(48*640 + 16, 40, 48*640 + 16, 40, 2, 2));
}
