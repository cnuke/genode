...
struct Xoroshiro_128_plus_reseeding
{
	...
	public:
		...
		Query_result query(Byte_range_ptr const &range)
		{
			...
			Xoroshiro_128_plus &xoroshiro = *_xoroshiro;

			/*
			 * For now we fill the supplied buffer up one 8 byte at
			 * a time and pay for the overhead.
			 */

			uint64_t const rounds    = buffer.num_bytes / sizeof(uint64_t);
			uint64_t const remainder = buffer.num_bytes % sizeof(uint64_t);

			char *dst = buffer.start;
			for (uint64_t i = 0; i < rounds; i++) {
				uint64_t const value = xoroshiro.value();
...
struct Vfs::Xoroshiro_file_system : Single_file_system
{
	...
	{
		return config.attribute_value("seed_file", File_path("/dev/entropy"));
	}
