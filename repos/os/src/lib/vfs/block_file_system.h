...
struct Vfs::Block_file_system
{
	...
	struct Block_job : Block::Connection<Block_job>::Job
	{
		...
		/* bytes handled for unaligned or partial requests */
		...
		class Block_vfs_handle : public Single_vfs_handle
		{
			private:
				...
				struct Read_handler
				{
					...
					Read_handler(size_t        const block_size,
					                 ...
					                 size_t                      &out_count)
					{
						...
						/*
						 * We constrain the request length to the amount that fits in
						 * the shared-memory and afterwards round up the number of blocks
						 * that is covered by the request seek offset and calculated length.
						 */
						...
						size_t const constrained_length =
							min(_length_limit - _helper.block_size(), dst.num_bytes);

						/* always round up to cover last block for partial requests */
						size_t const rounded_length =
							_helper.round_up(constrained_length + block_offset);
						...
						_job.construct(block, dst.start, constrained_length, block_offset, op);
				...
				struct Write_handler
				{
					                                  ...
					                                  size_t                     &out_count)
					{
						...
						if (_job->operation().type == Block::Operation::Type::WRITE) {
							out_count = _job->actual_length ? : _job->bytes_handled;
							_job.destruct();
						...
						size_t const partial_length =
							block_offset ? min(_helper.block_size() - block_offset, length)
							             : length;
					...
					Write_handler(size_t        const block_size,
					              block_count_t const block_count,
					              size_t        const length_limit)
					:
						_helper { block_size },
					                   ...
					                   size_t                      &out_count)
					{
						...
						/*
						 * We constrain the request length to the amount that fits in
						 * the shared-memory and afterwards round down the number of blocks
						 * that is covered by the request seek offset and calculated length.
						 */
						...
						size_t const constrained_length =
							min(_length_limit - _helper.block_size(), src.num_bytes);

						/* round down to handle partial requests later on */
						size_t const rounded_length =
							_helper.round_down(constrained_length + block_offset);
						 ...
						 * job was in charge of reading the block for unaligned or
						 * partial requests in.
						 */
						...
						/*
						 * If the request is unaligned or partial (non-full block)
						 * we first have to issue a READ request for the block covering
						 * the request.
						 */
						...
						if (!rounded_length && constrained_length < _helper.block_size())
							return _handle_unaligned_or_partial(block, block_number);
						...
						_job.construct(block, const_cast<char*>(src.start), constrained_length,
						               block_offset, op);
				...
				struct Sync_handler
				{
					...
					Sync_handler(block_count_t const block_count)
					: _block_count { block_count } { }
			...
			public:
				...
				:
					...
					_read_handler     { info.block_size, info.block_count, length_limit },
					_write_handler    { info.block_size, info.block_count, length_limit },
					_sync_handler     { info.block_count },
					_info             { info },
					_block            { block }
				{ }
				...
				Write_result write(Const_byte_range_ptr const &src, size_t &out_count) override
				{
					...
					/*
					 * Since there is no WRITE_QUEUED result value we issue
					 * WRITE_ERR_WOULD_BLOCK for this case and trick the
					 * caller, e.g. libc¸ in calling us again.
					 */
