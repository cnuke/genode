...
struct Vfs::Block_file_system
{
	...
	struct Block_job : Block::Connection<Block_job>::Job
	{
		...
		Block_job(Block::Connection<Block_job> &conn,
		           ...
		           file_size        const  initial_read_offset = 0)
		:
		...
		file_size read_space_remaining() const {
			...
			private:
				...
				struct Read_handler
				{
					                 ...
					                 size_t                      &out_count)
					{
						 ...
						 */
						_job.construct(block, dst.start, dst.num_bytes, op, block_offset);
