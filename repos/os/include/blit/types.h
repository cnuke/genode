...
namespace Blit {
	...
	static Rect snapped_to_16x16_grid(Rect r)
	{
		return Rect::compound(Point { .x =   r.x1()       & ~0xf,
		                              .y =   r.y1()       & ~0xf },
		                      Point { .x = ((r.x2() + 16) & ~0xf) - 1,
		                              .y = ((r.y2() + 16) & ~0xf) - 1 });
	}
	...
	{
		/* surface size must be divisible by 16 */
		if (!aligned(surface.size().w, 4) || !aligned(surface.size().h, 4))
			return;
		...
		Rect const src_rect = snapped_to_16x16_grid(rect);
		...
		/* coordinates converted to 16x16 units */
		unsigned const src_w = texture.size().w >> 4,
		               dst_w = surface.size().w >> 4,
		               w     = src_rect.area.w  >> 4,
		               h     = src_rect.area.h  >> 4;
