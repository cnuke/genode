...
static int claim_iface(struct usb_interface * iface)
{
	...
	urbs->in_delete = 0;
	return usb_driver_claim_interface(&skel_driver, iface, urbs);
...
static void release_iface(struct usb_interface * iface)
{
	...
	usb_driver_release_interface(&skel_driver, iface);
}
	...
	for (;;) {
		...
		for (; i < num; i++) {
			...
			if (usb_rpc_args.call == CLAIM) {
				...
				printk("set drivers iface %p to %p\n", iface, usb_get_intfdata(iface));
			} else {
				release_iface(iface);
			}
		}
                        ...
                        void * buf, unsigned long size, void * data)
{
	...
	if (!err) {
		...
		if (!usb_interface_claimed(iface)) {
			printk("Error: USB client uses interface without being claimed!\n");
			claim_iface(iface);
		}
		urbs = usb_get_intfdata(iface);
...
static int usbnet_init(void)
{
	...
	if ((err = usb_register(&skel_driver)))
		return err;
