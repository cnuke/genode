...
static int user_register_fb(struct drm_client_dev * dev, struct fb_info * info,
...
{
	return (uint64_t)g->max_width * (uint64_t)g->height <
                           ...
                           struct drm_display_mode * const min_mode)
{
	...
	bool                           max_resolution_enforcement = false;
	...
	drm_client_for_each_connector_iter(connector, &conn_iter) {
		...
		if (conf_mode.max_width && conf_mode.max_height) {
			max_resolution_enforcement = true;
	...
	if (max_resolution_enforcement) {
		/* request to be enforced max resolution if previous lookups failed */
		struct genode_mode conf_mode = { };
		lx_emul_i915_connector_config("dummy", &conf_mode);
		if (conf_mode.max_width && conf_mode.max_height) {
			prefer->hdisplay   = conf_mode.max_width;
			prefer->vdisplay   = conf_mode.max_height;

			/*
			 * If the to be enforced resolution is below the minimum
			 * resolution of all usable connectors, e.g. just eDP with 4K,
			 * and the to be enforced resolution is like 2K ...
			 * than we are in trouble. The to be enforced mode will not be
			 * accepted by drm_mode_setcrtc, so fall back to use min_mode as 
			 * is (and thereby using potentially too much memory for
			 * fb allocation) and hope the best to not get a Out_of_ram
			 * exception ...
			 */
			if (connector_usable <= 1)
				return;

			/*
			 * If connector_usable > 1, we may have luck that potentially one
			 * connector fulfill the to be enforced resolution in the next loop
			 */
		}
	}
	...
	drm_client_for_each_connector_iter(connector, &conn_iter) {
		list_for_each_entry(mode, &connector->modes, head) {
			...
			if (max_resolution_enforcement) {
				if (mode_larger(prefer, mode)) {
					/* at least one connector may work now ... */
					*min_mode = *mode;
					return;
				}
...
static bool reconfigure(struct drm_client_dev * const dev)
{
	...
	if (report_fb)
		user_register_fb(dev, &report_fb_info, dumb_fb.fb_id);
