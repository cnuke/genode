...
static void report_mixer_state(bool report_mixer, Mixer &mixer, Genode::Env *env = nullptr)
{
	...
	if (!mixer_reporter.constructed() || !report_mixer)
		return;
...
static void configure_mixer(Genode::Env &env, Mixer &mixer, Genode::Xml_node config)
{
	...
	bool const report_mixer = config.attribute_value<bool>("report_mixer", false);
	report_mixer_state(report_mixer, mixer, &env);
}
...
extern "C" void notify_hp_sense(int const sense)
{
	...
	report_mixer_state(true, mixer, nullptr);
}
